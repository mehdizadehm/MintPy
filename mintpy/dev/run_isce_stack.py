...
isce.demSNWE            = 31.1, 32.8, 130.1, 131.9   #[S, N, W, E] in degree, auto for none
...
isce.numThread          = 8                          #[int>=1], number of total threads, auto for 8
...
def fill_and_translate_template_auto_value(tempDict, autoDict=AUTO_DICT):
    ...
    for key, value in iter(tempDict.items()):
        if value == 'auto':
            tempDict.pop(key)
...
def run_sh_file(sh_file, text_cmd=None):
    """Run shell file"""
    print('running {}'.format(os.path.basename(sh_file)))

    # change file permission
    cmd = 'chmod +x {}'.format(sh_file)
    print(cmd)
    os.system(cmd)
...
def prepare_dem(iDict):
    ...
    if iDict['demSource'] == 'gsi_dehm':
        ...
        dem_bbox = [np.rint(i).astype(int) for i in dem_bbox]
    ...
    # clean up
    cmd = 'rm -f demLat*.dem demLat*.dem.xml demLat*.dem.vrt'
    print(cmd)
    os.system(cmd)

    cmd = 'fixImageXml.py -i demLat*.dem.wgs84 -f'
    print(cmd)
    os.system(cmd)
...
def prepare_ALOS(iDict):
    ...
    # run unPack script
    run_sh_file('run_unPackALOS', text_cmd=iDict['text_cmd'])
    return
...
def prepare_ALOS2(iDict):
    ...
    # run unPack script
    run_sh_file('run_unPackALOS2', text_cmd=iDict['text_cmd'])
    return
...
def prepare_stack(iDict):
    ...
    if proc == 'topsStack':
        ...
        if iDict['numThread']:
            num_proc = int(int(iDict['numThread']) / OMP_NUM_THREADS)
            num_proc = max(num_proc, 1)
            iargs += ['--num-process', str(num_proc)]
...
def run_stack(iDict, run_file_dir='run_files'):
    ...
    for step_ind in range(step0, step1+1):
        print('\n\n'+'#'*50)
        run_sh_file(run_files[step_ind], text_cmd=iDict['text_cmd'])
